...
function resolveArtifact(root: string, artifact: Artifact, cache: Map<string, string | null>): string | null {
...
function validate(record: AdrRecord, ledger: AdrRecord[], root: string, thisRepo: string, cache: Map<string, string | null>) {
  ...
  if (record.symbols.length === 0 && record.endpoints.length === 0) {
    ...
    if (record.enforcementClass === 4) {
      ...
      if (!hasAny) {
        ...
        for (const artifact of local) {
          const failure = resolveArtifact(root, artifact, cache);
...
export function check(cwd: string): CheckReport {
  ...
  for (const record of records) {
    const { failures, unverifiedRepos } = validate(record, records, cwd, thisRepo, cache);
    results.push({
